...
# створимо функція, яка знаходить найменше значення у дереві використовуючи прямий обхід
def traversal_min(root):
    global values
    if root:
        values.append(root.val)
        traversal_min(root.left)
        traversal_min(root.right)
    result = min(values)
    return result


print(f'Мінімальне значення у дереві: \n {root} дорівнює {traversal_min(root)}')
